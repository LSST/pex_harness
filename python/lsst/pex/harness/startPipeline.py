...
def startPipeline(nodeList, pipelinePolicy, runId, createIoDirs=False, createDbTables=False):
    ...
    if createIoDirs:
        lsst.pex.logging.Trace("pex.harness.startPipeline", 3, "Creating I/O directories for run %s" % (runId,))
        ...
        lsst.pex.logging.Trace("pex.harness.startPipeline", 3, "Not creating I/O directories for run %s" % (runId,))
    
    # create database tables for this run
    if createDbTables:
        lsst.pex.logging.Trace("pex.harness.startPipeline", 3, "Creating database tables for run %s" % (runId,))
        ...
        lsst.pex.logging.Trace("pex.harness.startPipeline", 3, "Not creating database tables for run %s" % (runId,))

    lsst.pex.logging.Trace("lsst.pex.harness.startPipeline", 3, "pipelineDir=%s" % (pipelineDir,))

    nnodes, ncores = parseNodeList(nodeList)
    lsst.pex.logging.Trace("lsst.pex.harness.startPipeline", 3, "nnodes=%s; ncores=%s; nslices=%s" % \
        (nnodes, ncores, ncores-1))
    
    lsst.pex.logging.Trace("lsst.pex.harness.startPipeline", 3, "Running mpdboot")
    subprocess.call(["mpdboot", "--totalnum=%s" % (nnodes,), "--file=%s" % (nodeList,), "--verbose"])
    
    time.sleep(3)
    lsst.pex.logging.Trace("lsst.pex.harness.startPipeline", 3, "Running mpdtrace")
    subprocess.call(["mpdtrace", "-l"])
    time.sleep(2)
    
    lsst.pex.logging.Trace("lsst.pex.harness.startPipeline", 3, "Running mpiexec")
    subprocess.call(
    ...
    time.sleep(1)    
    lsst.pex.logging.Trace("lsst.pex.harness.startPipeline", 3, "Running mpdallexit")
